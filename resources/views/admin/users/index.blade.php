                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr class="bg-gray-50">
                            ...
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">CI</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Código Login</th>
                        ...
                        @foreach($users as $user)
                            <tr class="hover:bg-blue-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    @if($user->user_type)
                                        <span class="font-medium">{{ ucfirst(str_replace('_', ' ', $user->user_type)) }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-3 py-1 inline-flex text-sm leading-5 font-bold rounded-full bg-gray-200 text-gray-800 font-mono">
                                        {{ $user->login_code ?? '----' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                    class="inline-flex items-center gap-1 p-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors duration-150">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
