                ...
                <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-5">
                    ...
                    <div id="passenger_type_section" style="display:none;">
                        ...
                        <select name="user_type" id="user_type"
                            ...
                            <option value="adult" {{ old('user_type', $user->user_type) === 'adult' ? 'selected' : '' }}>Regular (adulto) - 2.30 Bs</option>
                            <option value="senior" {{ old('user_type', $user->user_type) === 'senior' ? 'selected' : '' }}>Mayor de Edad - 1.00 Bs</option>
                            <option value="minor" {{ old('user_type', $user->user_type) === 'minor' ? 'selected' : '' }}>Menor de Edad - 1.00 Bs</option>
                            <option value="student_school" {{ old('user_type', $user->user_type) === 'student_school' ? 'selected' : '' }}>Estudiante Colegial - 1.00 Bs</option>
                            <option value="student_university" {{ old('user_type', $user->user_type) === 'student_university' ? 'selected' : '' }}>Estudiante Universitario - 1.00 Bs</option>
                        </select>
                    </div>
                    ...
                    <div id="student_university_fields" style="display:none;">
                        <div class="grid sm:grid-cols-3 gap-4">
                            <div class="sm:col-span-1">
                                ...
                                <input type="text" name="university_name" value="{{ old('university_name', $user->university_name) }}"
                                       class="mt-1 w-full border rounded-md px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Año Actual</label>
                                <input type="number" name="university_year" value="{{ old('university_year', $user->university_year) }}" min="1" max="7"
                                       class="mt-1 w-full border rounded-md px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Año Finalización</label>
                                <input type="number" name="university_end_year" value="{{ old('university_end_year', $user->university_end_year) }}" min="2025" max="2035"
                                       class="mt-1 w-full border rounded-md px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="ci" value="{{ old('ci', $user->ci) }}" maxlength="20"
                    ...
                    <div>
                        ...
                        <input type="text" name="nit" value="{{ old('nit', $user->nit) }}" maxlength="20"
                    ...
                    <div>
                        ...
                        <input type="text" name="login_code" value="{{ old('login_code', $user->login_code) }}"
                    ...
                    <div class="flex items-center gap-3">
                        <input id="active" type="checkbox" name="active" value="1" {{ old('active', $user->active) ? 'checked' : '' }}
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function togglePassengerSection() {
                ...
                if (role === 'passenger') {
                    ...
                    // No limpiar el valor en la página de edición
            ...
            roleSelect.addEventListener('change', function() {
                ...
                // Al cambiar el rol, también re-evaluar los campos de estudiante
                toggleStudentFields();
            });
            userTypeSelect.addEventListener('change', toggleStudentFields);

            // Ejecutar al cargar para preservar estado del usuario que se edita
            togglePassengerSection();
            toggleStudentFields();
        });
    </script>
